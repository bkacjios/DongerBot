...
dongerbot:hook("OnUserState", "Amer - Stop being drunk", function(event)
	local user = event.user

	if (user.channel or user.channel_from) and not user.channel_from then return end

	local channel = dongerbot:getChannels()[30] or dongerbot:getChannels()[6]

	-- TO DO: Register his ass
	if user.name ~= "Amer" then return end
