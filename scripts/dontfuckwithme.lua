hook.Add( 'OnUserChange', 'FUCKYOU', function(event)
	...
	if user == piepan.me and actor ~= user then
		if user.isServerMuted then
			actor:mute(true)
			user:mute(false)
			print(("[FuckOFF] %s attempted to mute the donger"):format(actor.name))
		elseif user.isServerDeafened then
			actor:deafen(true)
			user:deafen(false)
			print(("[FuckOFF] %s attempted to deafen the donger"):format(actor.name))
		elseif event.isChangedChannel and not actor:isMaster() then
			...
			print(("[FuckOFF] %s attempted to move the donger"):format(actor.name))
		end
	end
end )